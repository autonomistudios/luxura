...
function buildAiGeneratePrompt(ctx) {
  const {
    ...
    modelArchetypeDesc, poseDesc, expressionDesc, ageRangeDesc, shotTypeDesc, atmosphereDesc, stylingDesc,
  } = ctx;
  ...
  return [
    ...
    ageRangeDesc ? `\nAGE PROFILE [LOCKED]: ${ageRangeDesc}` : '',
    ...
    poseDesc ? `\nPOSE DIRECTION [LOCKED]: ${poseDesc}` : (slotPose ? `\nPOSE [LOCKED]: ${slotPose}` : ''),
    expressionDesc ? `\nFACIAL EXPRESSION [LOCKED]: ${expressionDesc}` : '',
    shotTypeDesc ? `\nSHOT TYPE & FRAMING [LOCKED]: ${shotTypeDesc}` : (slotFrm ? `\nFRAMING [LOCKED]: ${slotFrm}` : ''),
    ...
    userDirection
      ...
      : `\n\nSELF-AUDIT: □ ${anchorDesc} fully detailed? □ Background = "${bgLock}"? □ Skin tone correct? □ Model archetype correct? □ Lighting applied? □ 2 hands / 5 fingers each?`,
