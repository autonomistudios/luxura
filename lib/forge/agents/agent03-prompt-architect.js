...
export function classify(ctx) {
  ...
  if (fashnVTOImage)                     return MODES.VTO_BACKGROUND_REPLACE; // VTO present → scene-replace mode, garment pixels untouched
...
function buildAiGeneratePrompt(ctx) {
  const {
    ...
    modelIdentityDNA, modelHairDNA, dnaMap, skinToneDesc, hasClothingAnchor, mutation,
    userDirection,
  } = ctx;

  // Hair DNA: prefer keep-mode modelHairDNA, fall back to anchor-extracted dnaMap['HAIR']
  ...
  return [
    ...
    `\n\n${brief || ''}`,
    anchorRefNote || '',
    `\n\n══ MANDATORY OVERRIDES — THESE OVERRIDE ANY CONFLICTING CONTENT ABOVE ══`,
    `\nGENDER [ABSOLUTE LOCK]: Identical to reference subject — match the gender of the source image exactly.`,
    modelIdentityDNA ? `\nSUBJECT IDENTITY [ABSOLUTE LOCK]: The person in this image must precisely match this physical profile — face structure, skin tone, hair style and color, makeup, body shape, weight, proportions, and all permanent features — ${modelIdentityDNA.substring(0, 500)}` : '',
    ...
    userDirection
      ? `\n\nSELF-AUDIT: □ ${anchorDesc} fully detailed? □ Background = "${bgLock}"? □ Skin tone correct? □ 2 hands / 5 fingers each? □ Client direction ("${userDirection.substring(0, 60)}...") visually present?`
      : `\n\nSELF-AUDIT: □ ${anchorDesc} fully detailed? □ Background = "${bgLock}"? □ Skin tone correct? □ 2 hands / 5 fingers each?`,
...
function buildParts(mode, prompt, ctx) {
  ...
  switch (mode) {
    case MODES.AI_GENERATE:
      // Anchor ref image (face-free) → text prompt
      if (anchorRefImage) {
        return [
          { inlineData: { mimeType: anchorRefImage.mimeType, data: anchorRefImage.data } },
          { text: prompt },
        ];
      }
      return [{ text: prompt }];
    ...
    case MODES.VTO_BACKGROUND_REPLACE:
      // Single image: VTO output. Gemini modifies scene only — garment pixels untouched.
      // No garment reference image needed here — the VTO pixels ARE the garment.
      return [
        { inlineData: fashnVTOImage },
        { text: prompt },
      ];
