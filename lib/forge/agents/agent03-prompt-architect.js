...
function buildAiGeneratePrompt(ctx) {
  ...
  // Path C — a separate model photo is the identity anchor (Image 1). Identity comes from the
  // photo, so the casting/"fictional person" locks must NOT compete with (or forbid) it.
  const injected = !!ctx.modelIdentityImage;
  ...
  return [
    ...
    // Path C — injected model: identity is the photo (Image 1). One dominant lock; the casting
    // and "fictional person" instructions below are suppressed so they cannot fight the photo.
    injected ? `\nMODEL IDENTITY [ABSOLUTE LOCK — FROM IMAGE 1]: Image 1 IS the model. The subject in every one of the 6 images is THIS EXACT person — identical face, bone structure, skin tone, hair, body, and apparent age. Reproduce them faithfully; do NOT generate a new or fictional face, and do NOT beautify, age, de-age, slim, swap, or otherwise alter their appearance. The SAME person appears in all 6 plates — only pose, scene, and lighting change. Gender and skin tone are exactly as shown in Image 1.` : '',
    !injected ? `\nMODEL ARCHETYPE [LOCKED]: ${modelArchetypeDesc || 'High Fashion — editorial proportions'}` : '',
    (!injected && ageRangeExplicit)
...
function buildParts(mode, prompt, ctx) {
  ...
  switch (mode) {
    case MODES.AI_GENERATE: {
      const parts = [];
      // Injected model (Path C): lock the person from THIS photo (Image 1), before any
      // garment reference, so Gemini binds identity to it rather than inventing a face.
      if (ctx.modelIdentityImage) {
        parts.push({ text: '━━ IMAGE 1 — MODEL IDENTITY (THE PERSON) ━━\nThis photo shows the MODEL. Reproduce this EXACT person — face, bone structure, skin tone, hair, and body — as the subject in every image. Use it ONLY for who the person is; NOT for clothing, pose, or background.\n\n' });
        parts.push({ inlineData: { mimeType: ctx.modelIdentityImage.mimeType, data: ctx.modelIdentityImage.data } });
      }
      ...
      if (Array.isArray(ctx.anchorRefImages) && ctx.anchorRefImages.length > 1) {
        ...
        // Single garment anchor — label it as Image 2 when a model photo precedes it.
        if (ctx.modelIdentityImage) {
          parts.push({ text: '\n\n━━ IMAGE 2 — GARMENT (THE WARDROBE) ━━\nThis image shows ONLY the garment to reproduce — color, pattern, texture, and construction. Dress the person from Image 1 in this garment. Use it ONLY for clothing; never copy its model, face, or background.\n\n' });
        }
