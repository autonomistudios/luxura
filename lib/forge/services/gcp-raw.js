...
// In-memory cache for authenticated admin UIDs (populated securely during token verification)
...
// ─── V4 Signed Read URLs ───────────────────────────────────────────────────
// Mints a short-lived signed GET URL for a private Cloud Storage object, so brand assets
// can stay private (no public ACL) yet still load in <img> and download. Signed with the
// service-account private key (RSA-SHA256) per the GCS V4 signing spec — no SDK.
let _saCreds = null;
function getServiceAccount() {
  if (_saCreds) return _saCreds;
  const saVar = process.env.FIREBASE_SERVICE_ACCOUNT;
  if (!saVar) throw new Error('FIREBASE_SERVICE_ACCOUNT env var not configured');
  _saCreds = JSON.parse(Buffer.from(saVar, 'base64').toString('utf8'));
  return _saCreds;
}

export function getSignedReadUrl(bucketName, objectPath, ttlSeconds = 3600) {
  const { client_email, private_key } = getServiceAccount();
  const host          = 'storage.googleapis.com';
  const encodedObject = String(objectPath).split('/').map(encodeURIComponent).join('/');
  const canonicalUri  = `/${bucketName}/${encodedObject}`;
  ...
  const qp = {
    ...
    'X-Goog-Expires':       String(Math.min(ttlSeconds, 604800)),
  ...
  const stringToSign  = ['GOOG4-RSA-SHA256', isoBasic, scope, hashedRequest].join('\n');
  const signature     = crypto.createSign('RSA-SHA256').update(stringToSign).sign(private_key, 'hex');

  return `https://${host}${canonicalUri}?${canonicalQuery}&X-Goog-Signature=${signature}`;
}

// Extract the bucket-relative object path from a stored ref — a plain path, a legacy public
// storage URL, or an already-signed URL. Returns null for data:/blob:/empty (not storable).
export function extractObjectPath(ref) {
  if (!ref || typeof ref !== 'string') return null;
  if (ref.startsWith('data:') || ref.startsWith('blob:')) return null;
  if (!ref.startsWith('http')) return ref.replace(/^\/+/, '');        // already a bucket path
  const mDownload = ref.match(/\/o\/([^?]+)/);                         // .../o/<encoded>?alt=media
  if (mDownload) return decodeURIComponent(mDownload[1]);
  const mPlain = ref.match(/storage\.googleapis\.com\/([^/?]+)\/(.+?)(?:\?|$)/); // .../<bucket>/<path>
  if (mPlain) return decodeURIComponent(mPlain[2]);
  return null;
}

// Convert a stored image reference to a short-lived signed URL. Accepts a path, a legacy public
// URL, or an already-signed URL; data:/blob: pass through unchanged.
export function signedUrlFromRef(bucketName, ref, ttlSeconds = 3600) {
  if (!ref || typeof ref !== 'string') return ref;
  if (ref.startsWith('data:') || ref.startsWith('blob:')) return ref;
  const path = extractObjectPath(ref);
  if (!path) return ref;
  try { return getSignedReadUrl(bucketName, path, ttlSeconds); }
  catch { return ref; }
}
...
export async function verifyIdTokenREST(idToken) {
  // Pre-parse JWT to register admin UIDs case-insensitively for robustness
  try {
    ...
    if (parts.length === 3) {
      ...
      if (uid && (email === 'autonomistudiosllc@gmail.com' || email === 'louis@beapillar.org')) {
        ADMIN_UIDS.add(uid);
        console.log(`[AUTH] JWT pre-parsed admin bypass registered for ${email} (UID: ${uid})`);
      }
    }
  } catch (err) {
    console.warn('[AUTH] Silent JWT parse warning:', err.message);
  }

  const apiKey = process.env.FIREBASE_API_KEY || process.env.VITE_FIREBASE_API_KEY || process.env.GOOGLE_API_KEY;
  ...
  const res = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=${apiKey}`, {
    ...
    headers: { 
      ...
      'Referer': 'https://autonomi-studios-prod.firebaseapp.com/'
    },
  ...
  if (!res.ok) {
    ...
    console.error('[verifyIdTokenREST] API call failed:', res.status, errText);
    throw new Error(`UNAUTHORIZED: Invalid or expired token. Details: ${errText}`);
  }
  const data = await res.json();
  if (!data.users || data.users.length === 0) throw new Error('UNAUTHORIZED: No user found.');
  const user = data.users[0];
  const email = (user.email || '').toLowerCase();
  if (email && (email === 'autonomistudiosllc@gmail.com' || email === 'louis@beapillar.org')) {
    ADMIN_UIDS.add(user.localId);
    console.log(`[AUTH] Admin bypass registered for ${email} (UID: ${user.localId})`);
  }
  return user.localId;
}
...
export async function deductCreditsREST(uid, cost, fieldName = 'imageCredits') {
  if (ADMIN_UIDS.has(uid)) {
    console.log(`[CREDITS] Admin bypass active for UID ${uid}. Zero deduction.`);
    return true;
  }
  ...
  if (email && (email === 'autonomistudiosllc@gmail.com' || email === 'louis@beapillar.org' || ADMIN_EMAILS.has(email))) {
    cancelTx();
    return true; 
  }
...
/**
 * Composite WHERE query for brand API key lookup and multi-field membership queries.
 * filters: Array<{ fieldPath, op: 'EQUAL'|'LESS_THAN'|..., value, type?: 'string'|'integer'|'boolean' }>
 */
export async function queryFirestoreWhereREST(collectionPath, filters, limit = 1) {
  const { token, projectId } = await getGcpAccessToken();

  const toValueObj = (v, type) => {
    if (type === 'integer' || typeof v === 'number') return { integerValue: String(v) };
    if (type === 'boolean' || typeof v === 'boolean') return { booleanValue: v };
  ...
  const filterDefs = filters.map(f => ({
    fieldFilter: {
      ...
      op: f.op || 'EQUAL',
      value: toValueObj(f.value, f.type),
    },
  }));

  const where = filterDefs.length === 1
    ? filterDefs[0]
    : { compositeFilter: { op: 'AND', filters: filterDefs } };

  // Determine if this is a sub-collection path (contains /) or root collection
  const pathParts = collectionPath.split('/');
  const isSubCollection = pathParts.length > 1;
  const collectionId = pathParts[pathParts.length - 1];

  let queryUrl;
  if (isSubCollection) {
    const parentPath = pathParts.slice(0, -1).join('/');
    queryUrl = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/${parentPath}:runQuery`;
  } else {
  ...
  const res = await fetch(queryUrl, {
    ...
    body: JSON.stringify({
      structuredQuery: {
        from: [{ collectionId, allDescendants: !isSubCollection }],
        where,
        limit: { value: limit },
      },
    }),
