...
// Convert a stored image reference to a signed URL. Accepts a bucket-relative object PATH,
// or a legacy public storage URL (extracts the path), or a data:/blob: URL (returned as-is).
export function signedUrlFromRef(bucketName, ref, ttlSeconds = 3600) {
  ...
  let path = ref;
  // Legacy public URL forms → pull out the object path.
  const mDownload = ref.match(/\/o\/([^?]+)\?/);                       // .../o/<encoded>?alt=media
  const mPlain    = ref.match(/storage\.googleapis\.com\/(?:download\/storage\/v1\/b\/)?([^/]+)\/(.+?)(?:\?|$)/);
  if (mDownload) {
    path = decodeURIComponent(mDownload[1]);
  } else if (ref.startsWith('http') && mPlain) {
    path = decodeURIComponent(mPlain[2]);
  }
