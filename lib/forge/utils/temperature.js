...
import {
  ...
  CLOTHING_ANCHOR_TYPES,
} from '../constants.js';

// A locked garment (a SKU) is the product's consistency guarantee. In AI mode it must
// hold across all 6 plates — fidelity beats creativity. Superset of CLOTHING_ANCHOR_TYPES
// plus other garment categories so any apparel SKU pins the temperature low.
const GARMENT_PRECISION_ANCHORS = [
  ...CLOTHING_ANCHOR_TYPES,
  'DRESS', 'TOP', 'SKIRT', 'OUTERWEAR', 'SHOES', 'JACKET', 'COAT', 'SUIT', 'GOWN', 'JUMPSUIT',
];
...
export function calculateSlotTemperature(slotIndex, anchorList, isAiGen, framing) {
  ...
  //
  // GARMENT LOCK (highest priority): a locked garment/SKU must be reproduced identically
  // across all 6 plates. Per-slot variety comes from the Director's scene briefs (different
  // location, pose, light per slot), NOT from sampling temperature. Running a garment SKU at
  // the default ~1.05 guarantees print/colour/silhouette drift — so a garment anchor pins
  // temperature LOW (mirrors VTO mode, which holds pattern at 0.20–0.35).
  const hasGarmentAnchor = anchorList.some(a => GARMENT_PRECISION_ANCHORS.includes(a));
  if (hasGarmentAnchor) {
    let g = 0.50;
    if (isDetailFraming) g = 0.42;                         // close crops expose fabric — tighten further
    const microVariance = ((slotIndex % 5) - 2) * 0.015;   // ±0.03 for natural per-slot variation
    return Math.max(0.40, Math.min(0.66, g + microVariance));
  }
