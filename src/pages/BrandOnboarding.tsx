    ...
    <div className="min-h-screen bg-[#050505] text-white overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)',
          ...
          background: 'radial-gradient(ellipse at 50% 0%, rgba(197,162,83,0.07) 0%, transparent 55%)' }} />
      </div>
      ...
      <div className="relative min-h-screen flex flex-col items-center justify-center px-6 py-12">
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          ...
          <p className="text-[7px] font-mono tracking-[0.5em] uppercase text-[#C5A253] mt-1">B2B Operating System</p>
        </motion.div>
            ...
            <React.Fragment key={i}>
              <div className="flex flex-col items-center gap-2">
                <div className="w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all"
                  style={{
                    borderColor: i < step ? '#10B981' : i === step ? '#C5A253' : 'rgba(255,255,255,0.15)',
                    background: i < step ? '#10B98120' : i === step ? '#C5A25320' : 'transparent',
                    boxShadow: i === step ? '0 0 12px rgba(197,162,83,0.4)' : 'none',
                  }}>
                  {i < step
                    ? <Check size={12} style={{ color: '#10B981' }} />
                    : <span className="text-[9px] font-mono" style={{ color: i === step ? '#C5A253' : 'rgba(255,255,255,0.2)' }}>{i + 1}</span>
                  }
                </div>
                <span className="text-[6px] font-mono tracking-[0.3em] uppercase whitespace-nowrap"
                  style={{ color: i <= step ? '#C5A253' : 'rgba(255,255,255,0.2)' }}>{label}</span>
              </div>
              {i < STEPS.length - 1 && (
                  ...
                  {USE_CASES.map(uc => {
                    ...
                    return (
                      <button key={uc.id} onClick={() => toggleUseCase(uc.id)}
                        ...
                        style={{
                          background: selected ? 'rgba(197,162,83,0.08)' : 'rgba(255,255,255,0.02)',
                          border: selected ? '2px solid rgba(197,162,83,0.4)' : '2px solid rgba(255,255,255,0.07)',
                        }}>
                        <div className="w-5 h-5 rounded flex items-center justify-center flex-shrink-0 mt-0.5"
                          style={{ background: selected ? '#C5A253' : 'rgba(255,255,255,0.06)', border: selected ? 'none' : '1px solid rgba(255,255,255,0.1)' }}>
                    ...
                    return (
                      <motion.button
                        ...
                        style={{
                          ...
                          boxShadow: selected
                            ? '0 0 30px rgba(197,162,83,0.30), 0 0 60px rgba(197,162,83,0.10)'
                      ...
                      >
                        ...
                        {selected && (
                          <motion.div
                            ...
                            animate={{ opacity: 1 }}
                            style={{
                              background: 'linear-gradient(135deg, rgba(197,162,83,0.08) 0%, transparent 50%, rgba(197,162,83,0.04) 100%)',
                            }}
                          />
                        )}
                        ...
                        {selected && (
                          <motion.div
                            ...
                            style={{
                              background: '#C5A253',
                              boxShadow: '0 0 12px rgba(197,162,83,0.6)',
                            }}
                          >
                        ...
                        <div
                          ...
                          style={{
                            fontSize: 48,
                            color: selected ? 'rgba(197,162,83,0.18)' : 'rgba(255,255,255,0.07)',
                        ...
                        <div className="relative z-10 p-4 flex flex-col gap-2">
                          ...
                          <div className="flex flex-wrap gap-1 mb-1">
                            {pd.mood.map(m => (
                              <span
                                ...
                                style={{
                                  background: selected ? 'rgba(197,162,83,0.20)' : 'rgba(255,255,255,0.08)',
                                  color:      selected ? '#C5A253' : 'rgba(255,255,255,0.45)',
                                  border:     `1px solid ${selected ? 'rgba(197,162,83,0.35)' : 'rgba(255,255,255,0.1)'}`,
                                  transition: 'all 0.3s',
                                }}
                              >
                                {m}
                          ...
                          <div
                            ...
                            style={{
                              background: selected
                                ? 'linear-gradient(to right, rgba(197,162,83,0.6), transparent)'
              ...
              <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                ...
                <div className="flex flex-col gap-3 text-left">
                  ...
                  <button onClick={() => setInviteEmails(p => [...p, ''])}
                    className="text-[8px] font-mono text-white/25 hover:text-[#C5A253] transition-colors self-start uppercase tracking-[0.3em]">
                    + Add another
                  </button>
                </div>
              </motion.div>
        ...
        <div className="flex items-center justify-between w-full max-w-2xl mt-10">
          ...
          {step < 3 ? (
            <button onClick={() => setStep(s => s + 1)} disabled={!canProceed[step]}
              ...
              style={{ background: '#C5A253', color: '#000', boxShadow: canProceed[step] ? '0 0 24px rgba(197,162,83,0.3)' : 'none' }}>
            ...
            <button onClick={handleSubmit} disabled={submitting}
              ...
              style={{ background: '#C5A253', color: '#000', boxShadow: '0 0 24px rgba(197,162,83,0.3)' }}>
